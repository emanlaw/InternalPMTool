...
def search_cards():
    try:
        ...
        if firebase_config.db is not None:
            ...
            # Convert Firebase card IDs to display numbers for filtering
            for card in cards:
                if isinstance(card['id'], str) and card['id'].startswith('CARD-'):
                    try:
                        card['display_id'] = int(card['id'].split('-')[1])
                    except (ValueError, IndexError):
                        card['display_id'] = card['id']
                else:
                    card['display_id'] = card['id']
        else:
        ...
        # Project filter - handle both string and integer project IDs
        if project_filter:
            filtered_cards = [c for c in filtered_cards if str(c['project_id']) == str(project_filter)]
...
def export_excel():
    try:
        ...
        if firebase_config.db is not None:
            # Use Firebase
            cards = firebase_service.get_all_cards()
            projects = {p['id']: p['name'] for p in firebase_service.get_all_projects()}
        else:
            # Fallback to JSON data
            data = load_data()
            cards = data['cards']
        ...
        for card in filtered_cards:
            # Get display ID for Excel export
            display_id = card['id']
            if isinstance(card['id'], str) and card['id'].startswith('CARD-'):
                try:
                    display_id = int(card['id'].split('-')[1])
                except (ValueError, IndexError):
                    display_id = card['id']
